...
function buildItems(pool: ImageItem[], seg: number): ItemDef[] {
  // Dynamically start xCols so they perfectly wrap around based on seg count
  const startX = -seg + (seg % 2 === 0 ? 1 : 0);
  const xCols = Array.from({ length: seg }, (_, i) => startX + i * 2);
  
  // Reduce the number of rows for a cooler, less crowded look
  const evenYs = [-2, 0, 2];
  const oddYs = [-1, 1];
